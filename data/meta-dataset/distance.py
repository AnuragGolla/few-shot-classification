...
def mahalanobis_distance(support, support_mean, query, beta=1):
    ...
    if Shot == 1:
        ...
        # query_dif = query_dif.unsqueeze(1).expand(-1,5,-1) # 5, 5, 64
        # inv_cov_matrix = inv_cov_matrix.unsqueeze(0).expand(5, -1, -1, -1) # 5, 5, 64, 64

        # query_dif = query_dif.view(25,1,Depth)
        # inv_cov_matrix = inv_cov_matrix.view(25, Depth, Depth)
