...
def emd_distance(support, support_mean, query):
    ...
    support_mean_normalized = (support_mean + torch.min(support_mean, 2).unsqueeze(-1)) / torch.max(support_mean, 2).unsqueeze(-1) # 32, 5, 64
    support_mean_normalized /= torch.sum(support_mean_normalized, -1, keepdim=True) # 32, 5, 64

    # normalize query
    query_normalized = (query + torch.min(query, 2).unsqueeze(-1)) / torch.max(query, 2).unsqueeze(-1) # 32, 5, 64
